...
from ..service.commands import CatalogQuery, ReactCommand, SavePreferencesCommand
from ..service.entities import Category, Event, Preferences, ServiceConfig, User
from ..service.results import (AuthenticationResult, CatalogEvent, CatalogResult, HealthResult,
                               PreferencesResult, ReactionResult, RecommendedEvent, SyncResult)
from .api.schemas import (AuthOut, CatalogEventOut, CatalogOut, CategoryOut, ConfigOut, EventOut,
                          HealthOut, PreferencesInput, PreferencesOut, ReactionInput, ReactionOut,
                          SyncOut, UserOut)


def preferences_to_command(body: PreferencesInput) -> SavePreferencesCommand:
    return SavePreferencesCommand(city=body.city, preferences=Preferences(
        categories=tuple(body.categories), budget_max=body.budget_max, companion=body.companion,
        preferred_days=body.preferred_days, preferred_time=body.preferred_time))
...
def catalog_to_query(q: str, date_mode: str, selected_date: date | None,
                     categories: list[str], free_only: bool, budget_max: int | None,
                     time: str, limit: int, offset: int) -> CatalogQuery:
    return CatalogQuery(q=q, date_mode=date_mode, selected_date=selected_date, categories=tuple(categories),
                        free_only=free_only, budget_max=budget_max, evening=time == 'evening',
                        limit=limit, offset=offset)


def user_to_response(user: User) -> UserOut:
    return UserOut(id=user.id, first_name=user.first_name, city=user.city,
                   onboarding_completed=user.onboarding_completed)


def auth_to_response(result: AuthenticationResult) -> AuthOut:
    return AuthOut(access_token=result.access_token, token_type='bearer', user=user_to_response(result.user))
...
def preferences_to_response(result: PreferencesResult) -> PreferencesOut:
    ...
    return PreferencesOut(city=result.city, categories=list(preference.categories), budget_max=preference.budget_max,
                          companion=preference.companion, preferred_days=preference.preferred_days,
...
def event_response_values(event: Event, reasons: tuple[str, ...] = ()) -> dict:
    return {
        ...
        'provider': event.provider, 'reasons': list(reasons),
    }
...
def catalog_to_response(result: CatalogResult) -> CatalogOut:
    return CatalogOut(items=[catalog_event_to_response(item) for item in result.items],
                      total=result.total, has_more=result.has_more)


def configuration_to_response(config: ServiceConfig) -> ConfigOut:
    return ConfigOut(demo_mode=config.demo_mode, event_provider=config.event_provider,
                     max_bot_name=config.max_bot_name, demo_cities=list(config.demo_cities))
