...
class Services:
    def __init__(self, uow: UnitOfWorkPort, provider: EventProviderPort,
                 verifier: MaxVerifierPort, tokens: TokenPort, clock: ClockPort,
                 config: ServiceConfig, sync_state: SyncState):
        ...
        self.recommendations = RecommendationService(uow, self.sync, clock, config.event_provider)
